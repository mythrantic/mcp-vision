...
Clone the repo:
```bash
git clone git@github.com:mythrantic/mcp-vision.git
```
...
"mcpServers": {
  "mcp-vision": {
    "command": "docker",
    "args": ["run", "-i", "--rm", "--runtime=nvidia", "--gpus", "all", "mythrantic/mcp-vision:latest"],
	"env": {}
  }
}
```
...
[mythrantic Internal] Push the Docker image to Docker Hub (requires DockerHub credentials):
```bash
make push-docker
```
...
### Acknowledgements
- This is an extension of the work done by Groundlight AI in their MCP Vision server, which can be found here: https://github.com/groundlight/mcp-vision
it was extensively modified and improved upon, but the original work provided a strong foundation to build upon.
