...
async def app_lifespan(server: FastMCP):
    ...
    logger.info(
        "Starting up MCP-vision server and loading the pipeline(s), this may take a few minutes..."
    )
    try:
...
def load_hf_objdet_pipeline(model_name: str):
    ...
    print(
        f"Loaded zero-shot object detection pipline for {model_name} in {time.time() - start:.2f} seconds."
    )
    return {"model_name": model_name, "model": objdet_model}


def locate_object_bboxes(
...
@mcp.tool()
def locate_objects(
    image_path: str, candidate_labels: list[str], hf_model: str | None = None
) -> str:
...
@mcp.tool()
def zoom_to_object(
    image_path: str, label: str, hf_model: str | None = None
) -> MCPImage:
